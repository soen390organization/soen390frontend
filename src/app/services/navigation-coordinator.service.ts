...
export class NavigationCoordinatorService {
  ...
  async getCompleteRoute(start: Location, destination: Location): Promise<CompleteRoute> {
    // If both locations are outdoor, use the OutdoorRoutingStrategy.
    if (start.type === 'outdoor' && destination.type === 'outdoor') {
      const segment: RouteSegment = await this.outdoorStrategy.getRoute(start, destination);
